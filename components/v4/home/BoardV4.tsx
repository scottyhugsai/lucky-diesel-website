...
export function BoardV4({ stats }: { stats: BuildStats }) {
  ...
  return (
    <section aria-labelledby="board-v4-heading" className={`${SECTION} border-t border-line`}>
      <div className={WRAP}>
        <SectionHead id="board-v4-heading" index="Dyno board" title="What we made" href="/builds" linkLabel="All builds" />
        <ol className="v4-stagger mt-8">
          {rows.map((build) => (
            <li key={build.slug} className="relative isolate border-b border-line">
              ...
              <Link href={`/builds/${build.slug}`} className="group grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 py-5 transition-colors hover:bg-carbon-2/60 sm:gap-6">
