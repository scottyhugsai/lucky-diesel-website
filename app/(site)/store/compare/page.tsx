  ...
  if (chosen.length < 2) {
    return (
      <QuietPage
        eyebrow="Compare"
        title="Side by side"
        line="Every row says what the spec actually means, so you are not expected to already know."
        status={`Nothing to compare yet. Tick two to ${MAX_COMPARE} parts on the parts page and press Compare.`}
        onward={[
          { href: '/store/products', label: 'All parts', line: 'Tick the boxes on any two cards.' },
          { href: '/fitment', label: 'What fits your truck', line: 'Narrow it down before you compare.' },
          { href: BUSINESS.phoneHref, label: `Call ${BUSINESS.phoneDisplay}`, line: 'Or just ask which one you want.', external: true },
        ]}
      />
    );
  }
