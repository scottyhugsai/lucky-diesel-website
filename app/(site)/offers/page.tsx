            ...
            {offers.map((page) => (
              <li key={page.id} className="grid content-between gap-6 rounded-md border border-line bg-carbon-2 p-6 transition-colors hover:border-clover/50">
                <div className="grid gap-3">
                  ...
                  {page.offer!.endsAt && <p className="text-sm text-chalk/80">Ends {dateLabel(page.offer!.endsAt)}</p>}
                </div>
                <Link href={`/l/${page.slug}#claim`} className="btn-go display inline-flex w-fit items-center gap-2 rounded-sm px-5 py-3 text-lg not-italic">
                  Claim it <ArrowRight className="size-5" aria-hidden="true" />
                </Link>
              </li>
            ))}
          </ul>
