  ...
  if (events.length === 0) {
    return (
      <QuietPage
        eyebrow="Events"
        title="Dyno days"
        line="Bring the truck. Get real numbers."
        status="Nothing on the calendar right now. Call and we will put you on the list for the next one."
        onward={[
          { href: BUSINESS.phoneHref, label: `Call ${BUSINESS.phoneDisplay}`, line: 'Ask when the next one is.', external: true },
          { href: '/book', label: 'Book the shop', line: 'A dyno pull without waiting for a day.' },
          { href: '/builds', label: 'Builds', line: 'What other trucks made.' },
        ]}
      />
    );
  }
