...
import { activate, createCampaign, pause, publishToCampaign, resubmitCampaign, saveGuard } from './actions';
...
export default async function CampaignsPage() {
  ...
  const db = adminDb();
  const [{ data: rows }, { data: creatives }, guards] = await Promise.all([
  ...
  const campaigns: CampaignView[] = (rows ?? []).filter((c) => isCampaignPlatform(c.platform)).map((c) => {
    const audience = (c.audience ?? {}) as { radiusMiles?: number };
    return {
      ...
      startsOn: c.starts_on, endsOn: c.ends_on, simulated: c.simulated, notes: c.notes, radiusMiles: audience.radiusMiles ?? null, publications: c.ad_publications.length,
    };
  });
